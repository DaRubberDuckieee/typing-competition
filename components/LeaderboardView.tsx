...
export function LeaderboardView({
  ...
  const load = useCallback(async (s: Scope) => {
    ...
    try {
      const r = await fetch(`/api/leaderboard?scope=${s}`, { cache: 'no-store' });
      if (!r.ok) return;
    ...
    <section className="lb-view">
      {showHeader && (
        <header className="lb-view__head">
          ...
          <h1 className="h1" style={{ marginTop: 12 }}>Typing Competition</h1>
        </header>
      )}
      ...
      {/* CTA pulled ABOVE the table so booth players never have to scroll
          to start a new race. */}
      <div className="lb-view__cta lb-view__cta--top row-wrap" style={{ justifyContent: 'center' }}>
        {lockToLane ? (
          onPlayNow ? (
